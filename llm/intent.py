...
_EN_STRONG_TERMS = [
    ...
    # REMOVED ambiguous attendance phrases that collide with leave/scheduling
    # r"\battend(?:ing)?\s+(?:class|lesson)\b", r"\bgo to class\b",
...
_ZH_HK_STRONG_TERMS = [
    ...
    # REMOVED ambiguous attendance markers that collide with leave/scheduling
    # r"上課|上堂|返學|返課",
    r"公眾假期|假期",
]
_ZH_HK_WEAK_TERMS = [
...
_NEG_EN = [
    ...
    r"\bhow\s+much\b", r"\bper\s+(?:hour|hr|minute|min)s?\b",
...
_SCHED_EN = [
    ...
    r"\bwon'?t\s+be\s+able\s+to\s+attend\b",
    r"\bwill\s+be\s+away\b",
    r"\b(?:be|is|are)\s+away\b",
]
